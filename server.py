...
class GameManager:
    ...
    def start(self, users):
        ...
        self.users.sendMessangeToAll('/start')

    def getIndex(self, username):
        conn, addr = users[username]
        try:
            index = self.players.index(conn)
        except ValueError:
            pass
        return index
...
class UserManager:  # 사용자관리 및 채팅 메세지 전송을 담당하는 클래스
    ...
    def gameStart(self):
        self.gamemanager.start(self.users)
...
class MyTcpHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):  # 클라이언트가 접속시 클라이언트 주소 출력
        try:
            ...
            if len(self.userman.users) == 4:
                self.userman.sendMessageToAll('/start')
                self.userman.gameStart()
